...
#Convertir en dataframes para utilizar ggwordcloud

#Non-fiction

df_count_non_fiction <- data.frame(names(count_non_fiction),count_non_fiction,
                                   row.names = NULL)
colnames(df_count_non_fiction) <- c("Words","Frequency")

#Sequential art

df_count_seq_art <- data.frame(names(count_seq_art),count_seq_art,
                                   row.names = NULL)
colnames(df_count_seq_art) <- c("Words","Frequency")

#Fiction

df_count_fiction <- data.frame(names(count_fiction),count_fiction,
                                   row.names = NULL)
colnames(df_count_fiction) <- c("Words","Frequency")

#Young adult

df_count_young_adult <- data.frame(names(count_young_adult),count_young_adult,
                                   row.names = NULL)
colnames(df_count_young_adult) <- c("Words","Frequency")


#Nube de palabras


ggplot(df_count_non_fiction, aes(label = Words, size = Frequency)) +
  geom_text_wordcloud(max.words = 30) +
